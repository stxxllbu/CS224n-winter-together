#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @@@ Define original embedding (word, not char) for target, Only need change this file to accomodate the difference.
# The training ppl & loss will go to zero on the small training sample, BLEU on small test sample is 27.XX not 99.XX
# because test.json is designed to be characterized based. Search @@@ for diffs
"""
...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        # self.vocab = vocab
        self.ee = nn.Embedding(len(vocab.tgt), word_embed_size, 0) #@@@: define original embedding for target
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # padded size: (sen_len,
        # batch, m_word)
        target_padded_chars = self.vocab.src.to_input_tensor_char(target, device=self.device)  # padded size: (sen_len,
        # batch, m_word) 如果source都是characterized based，那target是不是也得是，不然languange model没法学习了

        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device) # size: (sen_len, batch)
        # target_padded = self.vocab.src.to_input_tensor(target, device=self.device)
        # 这里的target_padded是用来算loss的，给的是ground truth index
        ...
        #@@@ make sure you used target_padded not target_padded_char
        # combined_outputs = self.decode(enc_hiddens, enc_masks, dec_init_state, target_padded_char)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, dec_init_state = None, None
        X = self.model_embeddings_source(source_padded)  # (src_len, b, word_embed_size/filter) #
        # transpose以后，现在是（src_leb, b)， embed就是在后面加上 e
        # 如果不transpose，得到的是（b, src_len, e)
        X = pack_padded_sequence(X, torch.tensor(source_lengths))  # (b, src_len, e)???
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        target_padded = target_padded[:-1] #(tgt_len, b, m_word)
        ...
        Y = self.ee(target_padded) # @@@ Y embedding different Y = self.model_embeddings_target(target_padded)
        Y_ts = torch.split(Y, 1)
        for i in range(len(Y_ts)):  # timestep 23 totally, from 0 to 22
            Y_t = Y_ts[i]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Ybar_t is [Y_t o_prev]

        (dec_hidden, dec_cell) = self.decoder(Ybar_t, dec_state)  # (b, e+h) -> h by 1
        ...
        # e_t size=(b, src_len, 1) and squeeze to (b, src_len)
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        alpha_t = F.softmax(e_t, dim=1)  # (b, src_lens)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)  # (b, h*2)
        # details here: (b, 1, src_lens)   (b, src_lens, h*2)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            prev_hyp_ids = (top_cand_hyp_pos / len(self.vocab.tgt)).to(torch.int)
            hyp_word_ids = (top_cand_hyp_pos % len(self.vocab.tgt)).to(torch.int)
