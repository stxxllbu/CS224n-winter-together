...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished:
        # print([unfinished[i].buffer for i in range(len(unfinished))])
        batch_parse = unfinished[:batch_size]
        next_transition = model.predict(batch_parse)
        for parse, transition in zip(batch_parse, next_transition):
            # print('xxx')
            parse.parse_step(transition)
            if len(parse.buffer) == 0 and len(parse.stack) == 1:
                unfinished.remove(parse)
        # print('stack', [batch_parse[i].stack for i in range(len(batch_parse))])
        # print('buffer', [batch_parse[i].buffer for i in range(len(batch_parse))])
