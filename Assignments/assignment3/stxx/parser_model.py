...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # self.embeds = nn.Embedding(self.embed_size, self.vocab_size)
    ...
    def forward(self, w):
        ...
        embeddings = self.embedding_lookup(w) # (B, F) -> (B, F*E)

        x_W = torch.matmul(embeddings, self.embed_to_hidden_weight) # (B, F*E) * (F*E, H) = (B, H)
        h = F.relu(x_W + self.embed_to_hidden_bias) #(B, H) +
